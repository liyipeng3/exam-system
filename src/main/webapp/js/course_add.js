...
function saveCourse(nextStep,jump_url,type) {
    if($(".pay-money-switch").hasClass("switch-on")){
        $(".pay-money-input").val(1);
    }else {
        $(".pay-money-input").val(0);
    }
    if($(".isShowWatermark").is(":checked")){
        $(".isShowWatermark").val(1);
    }else {
        $(".isShowWatermark").val(0);
    }
	...
	$.ajax({
		...
		success: function(msg){
			...
			if(msg.success){
                if(nextStep){
                    window.location.href=jump_url+msg.bizContent.courseInfoId;
                    //alert(dataForm);
                }else {
                    //管理员体验地址
                    // var url = msg.bizContent.courseLink;
                    ...
                    if($("input[name='setProcess']").val()==1){
                        ...
                        showSelOk(msg.bizContent.courseInfoId, msg.bizContent.courseLink, '', msg.bizContent.courseLink, type, 0);
                    }
                }

                if(isNewCourse){
                    //gio 课程id，是否关联考试，是否付费课程，付费课程金额
                    var courseId_var = msg.bizContent.courseInfoId;
                    var ifLinkExam_var = nextStep ? '是' : '否';
                    var ifPaidCourse_var = $(".pay-money-input").val()==1 ? '是' : '否';
                    var paidCourseAmount_var = $("input[name=price]").val() ? parseFloat($("input[name=price]").val()) : 0;

                    ksxProbe.gioTrack('publishCourseSuccess', 1, {
                        'courseId_var': courseId_var,
                        'ifLinkExam_var': ifLinkExam_var,
                        'ifPaidCourse_var': ifPaidCourse_var,
                        'paidCourseAmount_var': paidCourseAmount_var
                    });
                }
            }else{
				alert("操作失败，请联系管理员！");
